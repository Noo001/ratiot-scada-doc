...
- `bug_cases.html` разделён на три главы по категориям (1. Ошибки кастомизации, 2. Ошибки OEM, 3. Ошибки платформы партнёра); группировка берётся из поля `**Категория:**` в `tests/BUG_CASES.md`, боковая навигация стала двухуровневой. Логика одинаковая в `tools/build_bug_cases.py` и `tools/build_bug_cases.pl` (pandoc-атрибут главы — `{#cat-N .chapter}`).
...
**Кейсы без тикета:** 12/14/15 (скрыты из отчёта).
