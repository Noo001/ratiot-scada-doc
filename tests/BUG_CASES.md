...
**Серьёзность:** High (до уточнения прав)  
**Тип:** Безопасность / RBAC

### Описание
При включённой саморегистрации (`usersSelfRegistration = true`) новый пользователь, зарегистрированный через Web UI, сразу попадает в интерфейс под названием **Admin Panel** с полным системным деревом (Пользователи, Приложения, Инструментальные панели, Тревоги и т.д.). Для процесса саморегистрации это необычно: обычно новый пользователь получает ограниченную роль (operator/viewer), без доступа к административным разделам.

> **Почему ожидались минимальные права:** стандартная практика безопасности — саморегистрация предназначена для операторов/наблюдателей. Открытый доступ к управлению пользователями, приложениями и системным деревом позволяет любому зарегистрированному пользователю видеть и потенциально менять конфигурацию сервера.
...
5. После успешной регистрации откроется **Admin Panel** с полным системным деревом.
...
**Результат:** HTTP 400 с сообщением «Контекст не найден: users.admin». Это может означать либо отсутствие прав, либо скрытие контекста, но в Web UI дерево пользователей отображается.

### Ожидаемый результат
Новый пользователь получает минимальные права (operator/viewer), без доступа к Admin Panel и управлению пользователями.

### Фактический результат
Новый пользователь сразу видит Admin Panel и системное дерево сервера.
...
### Что нужно уточнить
- Какая роль назначается саморегистрированному пользователю по умолчанию?
- Может ли `testuser02` создавать/редактировать других пользователей или изменять конфигурацию?
- Является ли «Admin Panel» дефолтным дашбордом для всех пользователей, а не признаком админ-прав?
