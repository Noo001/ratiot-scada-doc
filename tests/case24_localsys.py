#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Кейс 24: мастер — драйвер «Локальная Система», создание устройства."""
import time
...
with sync_playwright() as p:
    ...
    time.sleep(1.5)
    ok = click_option(page, "Локальная Система")
    print(f"«Локальная Система» выбрана: {ok}")
    time.sleep(1)
    page.locator("button:has-text('OK')").last.click()
    time.sleep(8)
    dump(page, "после OK")
    page.screenshot(path=str(OUT_DIR / "case24_64111_localsys.png"))
    browser.close()
    print("Готово.")
