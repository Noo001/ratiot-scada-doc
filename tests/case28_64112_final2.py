# -*- coding: utf-8 -*-
"""Кейс 28 (ASD-6521): чистый финальный сценарий.

1) удалить test (если есть)  2) создать test  3) добавить ресурс через дерево
4) упаковать -> ZIP  5) удалить ресурс, сохранить  6) обновить из ZIP
7) проверить «Результат обновления» и возврат ресурса  8) удалить приложение.
"""
import time
import shutil
...
def has_app(page, name="test"):
    body = page.locator("body").inner_text()
    return f"| {name} |" in body.replace("\t", " | ") or f"\n{name}\n" in body
...
def open_actions_menu(page, name="test"):
    goto_cc(page)
    if not page.locator(f"tr:has-text('{name}')").count():
        return False
...
def delete_app_if_exists(page, name="test"):
    if not open_actions_menu(page, name):
        print(f"Приложение {name} не найдено — удаление не требуется")
        return
    res = menu_item_coords(page, "Удалить")
    page.mouse.click(*res)
    time.sleep(4)
    page.screenshot(path=str(OUT / "case28_64112_80_delete_confirm.png"), full_page=True)
    body = page.locator("body").inner_text()
    print("Текст подтверждения:", body[:300].replace("\n", " | "))
    ...
    close_error(page)
    goto_cc(page)
    print(f"Приложение {name} удалено:", page.locator(f"tr:has-text('{name}')").count() == 0)
...
def add_resource_via_tree(page, dash_name="Устройства"):
    """Через дашборд приложения: «Добавить ресурс» -> Инструментальные панели -> панель -> OK -> OK."""
    # дашборд приложения
...
with sync_playwright() as p:
    ...
    # 0) очистка
    print("=== 0: удалить test, если есть ===")
    delete_app_if_exists(page)

    # 1) создать приложение
    print("=== 1: создать приложение test ===")
    errors_all.clear()
    create_app(page)
    print("JS-ошибок:", len(errors_all))

    # 2) добавить ресурс (дерево)
    print("=== 2: добавить ресурс ===")
    errors_all.clear()
    add_resource_via_tree(page)
    open_editor_resources(page)
    body = page.locator("body").inner_text()
    print("Ресурс «Устройства» добавлен:", "Устройства" in body)
    page.screenshot(path=str(OUT / "case28_64112_83_resource_added.png"), full_page=True)

    # 3) упаковать -> ZIP
    print("=== 3: упаковать -> ZIP ===")
    errors_all.clear()
    ...
    time.sleep(2)
    page.screenshot(path=str(OUT / "case28_64112_84_pack_zip.png"), full_page=True)
    packed = False
    try:
        ...
        print("Ошибка скачивания:", str(e)[:150])
    close_error(page)
    page.screenshot(path=str(OUT / "case28_64112_85_after_pack.png"), full_page=True)
    print("JS-ошибок (шаг 3):", len(errors_all))

    if packed:
        # 4) удалить ресурс, сохранить
        print("=== 4: удалить ресурсы ===")
        errors_all.clear()
        open_editor_resources(page)
        ...
        page.screenshot(path=str(OUT / "case28_64112_86_after_delete.png"), full_page=True)
        close_error(page)

        # 5) обновить из ZIP
        print("=== 5: обновить из ZIP ===")
        errors_all.clear()
        ok = open_actions_menu(page)
        print("Меню ДЕЙСТВИЯ:", ok)
        ...
        time.sleep(2)
        page.screenshot(path=str(OUT / "case28_64112_87_file_set.png"), full_page=True)
        page.locator("button:visible", has_text="OK").last.click()
        time.sleep(25)
        page.wait_for_load_state("networkidle", timeout=60000)
        time.sleep(3)
        page.screenshot(path=str(OUT / "case28_64112_88_update_result.png"), full_page=True)
        body = page.locator("body").inner_text()
        has_window = "Результат обновления" in body
        print("Окно «Результат обновления» открылось:", has_window)
        # заполненность окна
        if has_window:
            seg = body.split("Результат обновления")[1][:600]
        ...
        # 6) проверить возврат ресурса
        print("=== 6: проверка возврата ресурса ===")
        open_editor_resources(page)
        page.screenshot(path=str(OUT / "case28_64112_89_resources_final.png"), full_page=True)
        body = page.locator("body").inner_text()
        print("Ресурс «Устройства» вернулся:", "Устройства" in body)

    # 7) очистка
    print("=== 7: удалить приложение test ===")
    delete_app_if_exists(page)
    page.screenshot(path=str(OUT / "case28_64112_90_after_cleanup.png"), full_page=True)
